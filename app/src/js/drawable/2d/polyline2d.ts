...
import { PathPoint2DType, PolylineType, ShapeType, State } from '../../functional/types'
 ...
 * polyline 2d label
 */
export class Polyline2D extends Label2D {
  /** array for vertices */
  private _points: PathPoint2D[]
  /** polyline label state */
  private _state: Polyline2DState
  /** mouse position */
  ...
  constructor () {
    super()
    this._points = []
    this._state = Polyline2DState.FREE
  ...
  public draw (context: Context2D, ratio: number, mode: DrawMode): void {
    ...
    if (mode === DrawMode.CONTROL || self._selected || self._highlighted) {
      ...
      } else if (self._state === Polyline2DState.CLOSED) {
        for (let i = 0; i < numPoints; ++i) {
  ...
  /**
   * reshape the polyline: drag vertex or control points
   * @param _end
   * @param _limit
   */
  ...
  /**
   * add a new vertex to polyline label
  ...
  public onMouseDown (coord: Vector2D): boolean {
    ...
    if (this._selected) {
      ...
      if (this._state === Polyline2DState.CLOSED &&
          this._highlightedHandle < 0) {
        // not click edge or point
        return true
      } else if (this._state === Polyline2DState.CLOSED &&
        this._highlightedHandle > 0) {
          // click point
        ...
        this._highlightedHandle === 0) {
        // drag edges
        this._state = Polyline2DState.MOVE
        this.editing = true
        this.toCache()
        return true
      }
  ...
  /**
   * convert this drawable polyline to a polyline state
   */
  public toPolyline (): PolylineType {
    const pathPoints: PathPoint2DType[] = new Array()
    for (const point of this._points) {
      if (point.type === PointType.MID) continue
      pathPoints.push(point.toPathPoint())
    }
    return makePolyline({ points: pathPoints })
  }
  ...
  public shapeObjects (): [number[], ShapeTypeName[], ShapeType[]] {
    ...
    return [this._label.shapes, [ShapeTypeName.POLYLINE_2D],
            [this.toPolyline()]]
  }

  /**
   * create new polyline label
   * @param _state
   * @param _start
   */
  public initTemp (state: State, _start: Vector2D): void {
    ...
    this._label = makeLabel({
      type: LabelTypeName.POLYLINE_2D, id: -1, item: itemIndex,
  ...
  /**
   * to update the shape of polyline
   * @param _shapes
   */
  public updateShapes (shapes: ShapeType[]): void {
    if (this._label) {
      const polyline = shapes[0] as PolylineType
      if (!_.isEqual(this.toPolyline, polyline)) {
